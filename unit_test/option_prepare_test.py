...
def test_build_default_launches(monkeypatch):
    ...
    assert "quay.io/pypa/manylinux2014" in kwargs["container"]["image"]
    ...
    assert "quay.io/pypa/musllinux_1_2" in kwargs["container"]["image"]
    assert kwargs["container"]["cwd"] == PurePosixPath("/project")
    assert kwargs["container"]["oci_platform"] == OCIPlatform.AMD64
...
"""
    ...
    kwargs = build_in_container.call_args_list[2][1]
    assert "quay.io/pypa/manylinux_2_28" in kwargs["container"]["image"]
    assert kwargs["container"]["cwd"] == PurePosixPath("/project")
    assert kwargs["container"]["oci_platform"] == OCIPlatform.AMD64
    identifiers = {x.identifier for x in kwargs["platform_configs"]}
    assert identifiers == {
